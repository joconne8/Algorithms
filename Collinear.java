...
class Collinear {
   ...
   *         in the worst case in O(N^3).
    ...
    //TODO: your code goes here.
		int count = 0;
		for(int i =0; i<=a1.length-1;i++) //nest the for loops
   ...
   *         worst case in O(N^2 lg(N)).
    ...
    //TODO: your code goes here.
		...
		for(int i = 0; i<a1.length; i++) 
		{
			...
			{
				int x3Value = (a1[i]*(Y2-Y3)) + (a2[j]*(Y3 - Y1));
				boolean isLine = binarySearch(a3, x3Value);
				if (isLine == true) //if this x point is in the array , then it is a line cuts down on memory accessign 3rd array
					count++;
			}
  ...
  //comment this out for submitty or test the main somehow ??
  /*public static void main(String args[]) //are we looking for user input ??? Yes
  {
	  ...
	  System.out.println("The number of non-horizonatal collinear lines is: " + countCollinear(a, b, c));
